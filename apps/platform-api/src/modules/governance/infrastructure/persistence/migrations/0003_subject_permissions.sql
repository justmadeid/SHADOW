-- migration-safety: allow-destructive ADR-006
...
CROSS JOIN (VALUES ('SUBJECT_VIEW'), ('SUBJECT_CREATE'), ('SUBJECT_UPDATE')) p(permission)
WHERE r.case_role IN ('OWNER', 'EDITOR', 'VIEWER')
  AND (p.permission = 'SUBJECT_VIEW' OR r.case_role IN ('OWNER', 'EDITOR'))
